...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
    ...
    if (UserDefaults.standard.object(forKey: "crummyToken") as? String) != nil {
      if let
        rootViewController = self.window?.rootViewController as? LoginViewController,
        let storyboard = rootViewController.storyboard {
          let homeViewController = storyboard.instantiateViewController(withIdentifier: "HomeView") as! UINavigationController
